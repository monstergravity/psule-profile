...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.fillText('My AI Portrait Collection', canvasWidth / 2, 150);

    ctx.font = `50px 'Roboto', sans-serif`;
    ctx.fillStyle = '#555';
    ctx.fillText('Generated with psule.app', canvasWidth / 2, 220);
    ...
    const imagesWithStyles = styleNames.map((name, index) => ({
        name,
    ...
    reversedImages.forEach(({ name, img }, reversedIndex) => {
